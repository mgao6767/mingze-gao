...
3. Building a PD model: data and pipeline
4. **Widget 2**: Information Value intuition
5. **Widget 3**: Logistic curve visualiser · **Widget 4**: ROC and threshold slider
...
::: {.callout-tip}
Everything runs in your browser. Nothing to install. Nothing to download. Slide numbers are in the footer, so if you get lost, shout a number.
:::
...
- **Low PD.** A default is surprising and large when it happens. The unexpected component dominates, so capital rises with PD.
...
loan_large$rating <- cut(
  loan_large$pd_pit,
  breaks = pd_breaks, labels = rating_labels,
...
::: {.callout-note}
`credit_data` now holds **`r format(nrow(credit_data), big.mark=",")`** simulated loans over **`r n_years`** years, with 9 rating grades and a realistic default distribution. The `set.seed(42)` above makes this reproducible, so rerunning my analysis gives the same tables.
:::
...
- **Highly correlated** predictors cause multicollinearity: unstable coefficients and fragile predictions out of sample.
...
2. **Correlation and redundancy control**: among survivors, drop one of any highly correlated pair (keep the more informative).
...
Set $p^G$ and $p^B$ for one bin. If goods and bads are proportionally the same, IV contribution is 0 and the bin carries no signal. Push them apart to see contribution grow.
:::
...
corr_df |>
  tt() |>
  style_tt(j = 1, align = "l") |>
  style_tt(j = 2:ncol(corr_df), align = "r")
```
...
if (nrow(high_corr_pairs) == 0) {
  ...
  high_corr_info <- purrr::map_dfr(seq_len(nrow(high_corr_pairs)), function(i) {
    ...
    tibble(
      var1 = var1, var2 = var2, correlation = corr_mat[var1, var2],
      keep = keep, drop = drop
    )
  })
...
- Bounded in $[0,1]$, so outputs are valid probabilities.
- Linear in *log-odds*, so coefficients have a clean interpretation.
...
::: {.callout-note title="Experiment"}
Drag $\beta_0$ and the curve shifts horizontally. Drag $\beta_1$ and the curve *tilts* (positive = default risk rises with $X$; negative = falls). The magnitude of $\beta_1$ controls how "sharp" the transition is.
:::
...
- The magnitudes are on the **log-odds** scale, so don't read them as probability effects directly.
...
- Basel III requires **long-run PDs**, that is multi-year averages, for capital. Pro-cyclicality is bad for stability.
...
::: {.callout-caution}
**APS 113 floor.** Every grade's long-run PD must be at least **0.05%** for capital. The AAA row above has PD = 0.00%, so for capital purposes we would floor it to 0.05%.
:::
...
::: {.callout-note}
Z-score is an accounting-data scorecard, with no market prices required. That's its appeal for private firms. Its weakness is that the coefficients were fit on a small sample of US manufacturers with bankruptcies filed between 1946 and 1965; they are not universal across countries, industries or eras.
:::
...
**In our transition matrix (@tbl-transition-matrix), only ~3% of old-AAA borrowers remain AAA under the new model, and about 50% are re-graded all the way down to BBB. About 49% of old-BBB loans drop to BB. Good news or bad?**

::: {.fragment}
**Red flag either way, and it needs a story.** A three-notch downgrade concentrated at the *prime* end of the book is exactly the kind of shift a regulator will ask about:

- **If the old AAAs really were prime**, the new model is broken at the high end, so investigate the coefficients driving low-PD scores.
- **If the old AAAs were never really AAA**, the old model was too generous and the bank was under-capitalised. This is a *correction*, but one that must come with a provisioning and capital-planning story.
...
::: {.fragment}
**Because K covers unexpected loss, not expected loss.** When default is almost certain, the loss is *expected*, so it belongs in provisioning (IFRS 9) rather than capital. The "surprise" component, which is what capital funds, shrinks as PD approaches 1.
:::
...
1. **RWA = K × 12.5 × EAD** is the business end of credit-risk modelling. Everything before it, from simulating to screening, fitting, calibrating and validating, feeds one number.
...
- BIS Basel framework [CRE32: IRB approach and risk components](https://www.bis.org/basel_framework/chapter/CRE/32.htm)
