...
::: {.callout-warning title="Silicon Valley Bank collapses, 10 March 2023"}
...
::: {.fragment}
SVB had loaded up on long-duration bonds when rates were near zero. When the Fed raised rates by over 4 percentage points in 2022, the market value of those bonds plummeted and wiped out nearly all of SVB's equity.
...
1. Where interest rate risk comes from: maturity transformation, refinancing and reinvestment risk.
2. How rates move: the RBA's cash rate and why forecasts fail.
3. **Model 1: the repricing model**: book value, focused on net interest income.
4. **Model 2: the duration model**: market value, focused on net worth.
5. The SVB case study: what each model would (and would not) have caught.

::: {.callout-note title="Discussion"}
You have interest rate risk too. If the RBA cut the cash rate by 1% tomorrow, would *you* be better or worse off? A student with a variable-rate mortgage cheers; one living off term-deposit interest groans; HECS holders shrug (it is indexed to CPI, not the cash rate). You have just described your personal repricing gap. Today we build the bank-sized version.
:::
...
Recall that in Week 1, we explained one of the key functions performed by financial institutions (especially depository institutions) is __maturity transformation__. For DIs, they take short-term deposits from depositors and make long-term loans to borrowers.
...
If an FI is financed by issuing long-term bonds and invests in short-term loans, given a change in interest rate, its liabilities' value would change by more than the change in its assets' value, thereby causing an impact on its net worth.
::::
...
- The RBA targets an inflation band of 2 to 3% per annum (in place since 1993) and uses the **cash rate** as its primary instrument.
...
These shifts in the policy stance, particularly when large and fast, generate significant interest rate volatility across the financial system.
::::
...
::: {.callout-important}
Notice the 2022-23 hiking cycle: the US Fed raised rates by over 500 basis points in under 18 months, the fastest tightening in four decades. This is the macroeconomic backdrop for the SVB collapse.
:::
...
::: {.callout-note title="How this maps to today's two models"}
The 2016 BCBS IRRBB standards measure the risk in two ways: the change in **net interest income** ($\Delta NII$) and the change in the **economic value of equity** ($\Delta EVE$), each under a set of six prescribed interest rate shock scenarios. These are the industrial-strength versions of exactly the two models you learn today: the repricing model is the hand-calculable $\Delta NII$, and the duration model is the hand-calculable $\Delta EVE$.
:::

::: {.callout-warning}
SVB was exempt from enhanced IRRBB stress testing requirements under US rules for banks below US\$250 billion in assets. After the collapse, US regulators proposed extending stricter standards to banks above US\$100 billion, but the proposals were never finalised and the supervisory agenda has since shifted toward streamlining. As of 2026, the gap SVB fell through has been narrowed by supervision in practice, not closed by rule.
:::
...
::: {.fragment}
**Answers.** Rate-sensitive within one year: the 1-year consumer loans, 3-month Treasury bills, and the 30-year floating-rate mortgage (assets); the 3-month CDs, 6-month commercial papers, and 1-year term deposits (liabilities). Not rate-sensitive: the 2-year loans, 3-year bonds, 20-year fixed mortgage, 2-year term deposits, and the non-interest-bearing demand deposits (their rate is zero and stays zero, though heavy withdrawals are a different problem, for Week 8).
:::
...
- Its [FY2024 annual report](https://www.sec.gov/Archives/edgar/data/1077428/000107742825000036/tcbi-20241231.htm) was filed on 11 February 2025 (you can find the latest filing yourself via EDGAR).
...
The bond's duration is 4.61 years. Note: coupon < YTM, so the bond trades at a discount: market price of about $915.73, not $1,000.
...
SVB sat below the US\$250B threshold for enhanced IRRBB stress testing. Post-collapse proposals to extend stricter rules to banks above US\$100B were never finalised, and the 2026 US supervisory agenda has shifted toward streamlining.
:::
::::
:::

## Research note: why didn't every bank fail in 2023?

Here is the puzzle. At the end of 2022, US banks were sitting on roughly \$600 billion of unrealised securities losses, and most banks run positive duration gaps. If the duration model were the whole story, the 2022-23 hiking cycle should have been a massacre. It wasn't. Why?

@DrechslerSavovSchnabl2021 supply the missing piece: the **deposit franchise**.

- Retail deposit rates barely move when the cash rate rises; banks have pricing power over sticky depositors (the *implicit interest* idea returns in Week 9).
- Sticky, cheap deposits therefore behave like a **long-duration liability**, even though they are contractually overnight money.
- A bank with a stable retail franchise is *naturally hedged*: when rates rise, the loss on long-term assets is offset by the rising value of below-market deposit funding. In the authors' phrase, maturity transformation *without* interest rate risk.

The catch: the hedge only works if the deposits stay. SVB's deposits were concentrated, uninsured and quick to leave, and in follow-up work the same authors show that once such deposits run, the franchise value vanishes exactly when the asset losses bite. The duration model told us SVB's *assets* were exposed; the deposit-franchise research explains why that exposure was fatal for SVB yet survivable for nearly everyone else.
...
- We have discussed [capital management and adequacy](https://mingze-gao.com/teaching/AFIN8003/2026S2/Week3/) last week.
...
::: {.callout-note}
Had SVB actively managed its duration gap, for example with interest rate swaps (pay fixed, receive floating), it could have hedged much of its exposure without selling the bonds. This is what most large banks do.
:::
...
1. **Interest rate risk is baked into banking**: maturity transformation creates refinancing risk (negative gap) and reinvestment risk (positive gap).
2. **The repricing model**: $\Delta NII = GAP \times \Delta R$ per maturity bucket. Simple and book-value based, but blind to market-value losses.
3. **Duration** is the PV-weighted average time of cash flows; it measures price sensitivity: $\frac{\Delta P}{P} = -MD \times \Delta R$.
4. **The duration model**: $\Delta E = -(D_A - D_L k) \times A \times \frac{\Delta R}{1+R}$. The leverage-adjusted duration gap drives the net-worth effect.
5. **SVB is the proof**: a manageable repricing gap coexisted with a fatal duration gap. Book values hid a market-value insolvency.
6. To immunise the **capital ratio** ($E/A$) rather than the **level** of equity ($E$), target $D_A = D_L$ instead of $D_A = D_L k$.
7. **The deposit franchise is the hidden hedge** [@DrechslerSavovSchnabl2021]: sticky, low-rate deposits act like long-duration funding, which is why most banks survived 2022-23. The hedge fails when deposits are uninsured and flighty, which is why SVB did not.
